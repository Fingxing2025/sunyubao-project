Create Codex pet row `running-right` for `sunyu-baobao`: exactly 8 full-body frames in one horizontal strip on flat pure user-selected #0000FF.

Use the attached canonical base for identity and the layout guide only for spacing. Same pet in every frame: Exact same human chibi boy as supplied canonical reference. Black fluffy hair, browline glasses, navy raglan shirt and sage-gray sleeves, navy trousers, ivory shoes. Fix real alternating run cycle, keeping everything else unchanged.. Preserve silhouette, face, palette, material, proportions, markings, and props.
